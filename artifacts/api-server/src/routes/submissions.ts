...
type QuestionLike = { type: string; correctAnswer: string | null; options: string | null; points: number };

function gradeAnswer(question: QuestionLike, studentAnswer: string): { isCorrect: boolean | null; pointsEarned: number } {
  if (MANUAL_TYPES.includes(question.type)) return { isCorrect: null, pointsEarned: 0 };
  ...
  if (question.type === "listening") {
    ...
    for (let i = 0; i < subQs.length; i++) {
      if ((studentSubs[String(i)] ?? "").trim().toLowerCase() === (subQs[i].correctAnswer ?? "").trim().toLowerCase()) correct++;
    }
    const ratio = correct / subQs.length;
    return { isCorrect: ratio === 1, pointsEarned: Math.round(question.points * ratio) };
  }
...
router.post("/submissions", requireAuth, async (req, res): Promise<void> => {
  ...
  const autoGraded = questionMap.size > 0 && Array.from(questionMap.values()).every(q => {
    if (MANUAL_TYPES.includes(q.type)) return false;
