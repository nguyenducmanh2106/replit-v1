...
function QuestionEditDialog({ q, open, onClose, onSave, saving }: {
  ...
  useEffect(() => {
    ...
    if (q.type === "reading") {
      ...
      setReadSubQs(rawSubs.length > 0
        ? rawSubs.map(sq => ({ question: sq.question ?? "", choices: Array.isArray(sq.choices) ? sq.choices : ["", "", "", ""], correctAnswer: sq.correctAnswer ?? "" }))
        : [newSubQuestion()]);
    }
    if (q.type === "listening") {
      ...
      setLisSubQs(rawSubs2.length > 0
        ? rawSubs2.map(sq => ({ question: sq.question ?? "", choices: Array.isArray(sq.choices) ? sq.choices : ["", "", "", ""], correctAnswer: sq.correctAnswer ?? "" }))
        : [newSubQuestion()]);
    }
  ...
  function buildPayload(): Partial<EditDraft> {
    ...
    if (qType === "reading") {
      return { ...base, content, passage: readPassage || null, options: JSON.stringify(readSubQs), explanation: explanation || null } as any;
    }
    if (qType === "listening") {
      return { ...base, content, audioUrl: lisAudioUrl || null, passage: lisTranscript || null, options: JSON.stringify(lisSubQs), explanation: explanation || null } as any;
    }
    ...
    return { ...base, content, explanation: explanation || null } as any;
  }
    ...
    switch (q.type) {
      ...
      case "video_interactive":
        return <VideoInteractiveForm videoUrl={videoUrl} setVideoUrl={setVideoUrl} timedQuestions={videoTimedQs} setTimedQuestions={setVideoTimedQs}
          explanation={explanation} setExplanation={setExplanation} />;
      case "essay":
        return <EssayForm content={content} setContent={setContent} explanation={explanation} setExplanation={setExplanation} />;
      default:
        return (
...
export default function AssignmentDetailPage() {
  ...
  const [settingsDraft, setSettingsDraft] = useState({ startTime: "", endTime: "", maxAttempts: 1, allowReview: false, courseId: null as number | null });
  ...
  function handleSaveQuestion(data: Partial<EditDraft>) {
    if (!editingQ) return;
    updateAQ({ assignmentId, questionId: editingQ.id, data: data as Record<string, unknown> }, {
      onSuccess: () => {
    ...
    <div className="space-y-6">
      ...
      <Dialog open={importTemplateOpen} onOpenChange={setImportTemplateOpen}>
        <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
          <DialogHeader><DialogTitle>Nhập câu hỏi từ bộ Quiz nguồn</DialogTitle></DialogHeader>
          <div className="space-y-2 py-2">
              ...
              templates.map(t => (
                <div key={t.id} className="flex items-center gap-3 p-3 rounded-lg border hover:border-primary/40 transition-colors">
                  ...
                  <Button size="sm" onClick={() => handleImportTemplate(t.id)} disabled={importingTemplate}>
                    {importingTemplate ? <RefreshCw className="w-4 h-4 animate-spin" /> : "Nhập"}
                  </Button>
                </div>
              ))
            )}
          </div>
          <DialogFooter><Button variant="outline" onClick={() => setImportTemplateOpen(false)}>Đóng</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  );
