...
function SentenceReorderInput({ items, value, onChange }: { items: string[]; value: string; onChange: (v: string) => void }) {
  ...
  const available = items.filter(item => !ordered.includes(item));
...
export default function AssignmentTakePage() {
  ...
  const setAnswer = (v: string) => { if (currentAQ) setAnswers((prev) => ({ ...prev, [currentAQ.questionId]: v })); };
  const currentAnswer = currentAQ ? (answers[currentAQ.questionId] ?? "") : "";
  const answeredCount = questions.filter(aq => {
    const ans = answers[aq.questionId] ?? "";
  ...
  const isAnswered = (questionId: number, qType: string) => {
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-64 border-r border-gray-200/80 bg-white/90 backdrop-blur-sm p-5 overflow-y-auto flex-shrink-0">
          ...
          <div className="space-y-1.5">
            {questions.map((aq, i) => {
              const answered = isAnswered(aq.questionId, aq.question?.type ?? "");
          ...
          <div className="max-w-3xl mx-auto">
            {currentQ && (
              <div className="space-y-6">
                ...
                {/* Question type input */}
                <div className="ml-16">
                  {currentQ.type === "mcq" && rawOptions.length > 0 && (
                  ...
                  {currentQ.type === "word_selection" && rawOptions.length > 0 && (
                    <WordSelectionInput options={rawOptions} value={currentAnswer} onChange={setAnswer} />
                  )}
